...
class MovieIntegrationTest(TestCase):
    # Ajoute un film en BD afin de pouvoir faire les tests dessus (fixtures)
    def setUp(self):
        Movie.objects.create(
            title='Fight Club',
            author='David Fincher',
            length=139,
            genre='Thriller psychologic')

    # Ajoute un film via le site, et vérifie que le film soit bien crée en BD
    def test_add_movie(self):
        ...
        # Act : Envoi d'une requête POST pour ajouter un nouveau film
        client = Client()
        response = client.post(reverse('add_movie'), data=new_movie_data)

        # Assert
        # Redirection vers la page de détail du film après son ajout en BD
        self.assertRedirects(
            response,
            reverse('movieInfo', args=[2]),
            status_code=302,
            target_status_code=200
            )
    ...
    def test_movie_info(self):
        # test seulement si les films ont correctement été
        # ajouté via la fixture dans la BD
    ...
    # édit un film et vérifie que le film soit
    # bien sauvegarder en BD après édition
